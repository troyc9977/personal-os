...
milestones:
  ...
  - { title: "Mid-sprint check (5+ pages drafted)",      target: 2026-05-15, done: false }
  - { title: "First draft to FADE OUT",                  target: 2026-05-17, done: false }
  - { title: "Revisions complete",                       target: 2026-05-20, done: false }
...
# Short film script — outline to polish in 13 days
...
dormant since WardForge took over the calendar (per `identity.md`). A
13-day sprint to a 5-15 page short film script doesn't reverse that — it
...
without eating the WardForge time the week actually requires.

## Approach

13 days, four phases — Outline, Draft, Revise, Polish. ~1.5 hours/day target.
...
- Eating WardForge customer-outreach time. If the Tuesday-morning outreach
  block is behind, screenplay drops in priority for the rest of that week,
  no negotiation. Substrate-as-avoidance check applies.
...
- Perfectionism on the first draft. The Draft phase ends at FADE OUT,
  ...
  collapse the phases.
- Mid-sprint check (May 15) showing fewer than 5 pages. Either the script
  is shorter than expected (acceptable) or the schedule is slipping
  (diagnose: time, energy, or block?). Don't let May 15 quietly pass
  without naming which.
- The sprint extending past May 21 "just to make it good." That's the
  failure mode this format is designed to prevent. Ship at 21, even
  imperfect; if a real second pass is wanted later, that's a separate
  goal file.

## Open

- The logline. Troy writes this before May 10 outline session begins.
- Final page count target. Spec says 5-15 pages; the beat sheet on May 10
  is the natural moment to commit to a number.
- Whether anything from this sprint feeds the longer-arc filmmaking project
  (currently paused per `identity.md`). Decide after May 21 with the
